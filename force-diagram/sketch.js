...
function setup() {
  ...
  cluster = new Cluster(277, height-300, new Vec2D(width/2, height/2));
...
function draw() {
  ...
  // LIA'S NOTE: MAKE THEM FADE AWAY ON A TIMER
  if (showPhysics) {
...
function keyPressed() {
  if (key == 'c' || key == 'C') {
    ...
    cluster = new Cluster(Math.floor(30), random(10, height-100), new Vec2D((width/2)-10, height/2));
  }
}
